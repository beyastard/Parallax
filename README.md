...
|     └── parallax/
|             ├── __init__.py
|             ├── configuration_prallax.py   # ParallaxConfig dataclass
|             └── modeling_parallax.py       # Parallax model definition
├── scripts/
|      ├── dataset/
...
├── checkpoints/                             # Checkpoints generated by program
│   └── parallax_v1/
│       ├── model_best.safetensors
│       └── meta_best.pt
├── logs/                                    # TensorBoard event files
├── LICENSE                                  # MIT License
└── README.md                                # About the Parallax architecture
```
...
As this was developed on a Windows 11 machine using Python 3.13, the following were used (modify for Linux)
```bash
C:\Python313\python -m venv .venv
.venv\Scripts\activate.bat
pip install xformers==0.0.34 torch==$TorchVersion torchaudio==2.10.0 torchvision==0.25.0 torchmetrics==1.0.3 -i https://download.pytorch.org/whl/cu130
pip install transformers safetensors tensorboard numpy tqdm datasets
pip install flash_attn-2.8.3+cu130torch2.10.0cxx11abiTRUE-cp313-cp313-win_amd64.whl
```

Or, you can use the `setup.bat` file to set up the environment as mine was during development.
